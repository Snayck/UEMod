...
#include "Enums.h"
// #include "Encoding/UtfN.hpp"  // Not available, commented out
...
namespace UC
{
	...
	public:
		inline std::string ToString() const
		{
			if (*this)
			{
				// Simple UTF-16 to UTF-8 conversion replacement
				std::string result;
				int size = WideCharToMultiByte(CP_UTF8, 0, Data, NumElements - 1, nullptr, 0, nullptr, nullptr);
				if (size > 0) {
					result.resize(size);
					WideCharToMultiByte(CP_UTF8, 0, Data, NumElements - 1, &result[0], size, nullptr, nullptr);
				}
				return result;
			}
